		...
		private function onNetStatus(event:NetStatusEvent):void
		{
			switch (event.info.code)
			{
				...
				case NetConnectionCodes.CONNECT_SUCCESS:
					...
					if (this.resource is DynamicStreamingResource) {
						var dsResource:DynamicStreamingResource = this.resource as DynamicStreamingResource;
						var streamItems:Vector.<DynamicStreamingItem> = dsResource.streamItems;

						for (var i:int = streamItems.length-1; i >= 0; i--) {
							CONFIG::LOGGING
							{
								logger.debug("FCSubscribe: " + streamItems[i].streamName);
							}
